...
def download_file(service, file_id, file_name, output_folder="downloads"):
    """
    Download a single file from Google Drive, ensuring nothing from the 'Outputs' folder is downloaded.
    """
    # Skip downloading if the file belongs to the 'Outputs' folder
    if "outputs" in output_folder.lower():
        print(f"Skipping download of file {file_name} in 'Outputs' folder.")
        return None
...
def download_folder(service, folder_id, output_folder="downloads"):
    """
    Download all files from a Google Drive folder, excluding anything from the 'Outputs' folder.
    """
    ...
    # Download each file, skipping those from the 'Outputs' folder
    for file in folder_files:
        # Check if the file belongs to the 'Outputs' folder by its parent folder name
        parent_query = f"'{file['id']}' in parents and name='Outputs'"
        parent_results = service.files().list(q=parent_query, fields="files(id, name)").execute()
        if parent_results.get('files', []):
            print(f"Skipping file: {file['name']} (belongs to 'Outputs' folder).")
            continue
...
templates = {
    ...
    "Diabetes+PGX": r"downloads\PGXXXX - Copy.pptx"
...
def upload_outputs_to_drive(output_folder, sample_id_to_process):
    service = authenticate_drive_api()
    ...
    if files:
        ...
        print(f"Found existing 'Outputs' folder with ID: {folder_id}")
    else:
        # Create a new folder named 'Outputs' in Google Drive if it doesn't exist
    ...
    # Check for an existing report for the specific sample ID
    query = f"'{folder_id}' in parents and name contains '{sample_id_to_process}' and trashed = false"
    results = service.files().list(q=query, fields="files(id, name)").execute()
    sample_files = results.get('files', [])

    # Delete the existing report for the sample ID if found
    if sample_files:
        for file in sample_files:
            service.files().delete(fileId=file['id']).execute()
            print(f"Deleted existing report for Sample ID: {sample_id_to_process} from Google Drive.")

    # Upload the new report for the specific sample ID
    for filename in os.listdir(output_folder):
        if sample_id_to_process in filename:
            ...
            if os.path.isfile(file_path):
                ...
                # Upload file to the Outputs folder
                file = service.files().create(body=file_metadata, media_body=media, fields='id').execute()
                print(f"Uploaded: {filename} for Sample ID: {sample_id_to_process}")


def main(sample_id_to_process):
    ...
    # Filter for the specific sample ID
    patient_data_filtered = patient_data[patient_data["Sample_ID"] == sample_id_to_process]
    if patient_data_filtered.empty:
        print(f"No data found for Sample_ID: {sample_id_to_process}")
        return

    # Iterate over the filtered DataFrame
    for _, row in patient_data_filtered.iterrows():
    ...
    # Read data from Excel file for recommendations
    recommendations_df = pd.read_excel(excel_path, sheet_name="Recommendations")

    # Filter recommendations for the specific sample ID
    recommendations_filtered = recommendations_df[recommendations_df["Sample_ID"] == sample_id_to_process]
    if recommendations_filtered.empty:
        print(f"No recommendations found for Sample_ID: {sample_id_to_process}")
        return

    # Process recommendations
    for _, patient_data in recommendations_filtered.groupby("Sample_ID"):
        ...
        for pptx_file in os.listdir(input_folder):
            if pptx_file.endswith(".pptx") and sample_id in pptx_file:
    ...
    upload_outputs_to_drive(output_folder, sample_id_to_process=sample_id_to_process)

if __name__ == "__main__":
    # Replace 'KHDEOMGPCSP8' with the sample ID you want to process
    sample_id_to_process = 'KHDEOMGPCSP11'
    main(sample_id_to_process)

    sample_id_to_process = 'KHDEOMGPCSP9'
    main(sample_id_to_process)

    sample_id_to_process = 'KHDEOMGPCSP10'
    main(sample_id_to_process)
